...
observation at very high spatial resolution (respectively at 8 mm and 80
mm). Taking advantage of their extremly high spatial resolution; the low
...
and benthic Bacillariophyceae (Diatoms), and validated using concomitant
...
area of 467 000 m². The model exhibited an accuracy of 96.4\% in
...
ot possoble to assess the influence of spatial resolution on the
classification outputs, showing a limited loss in seagrass detection up
to about 10m. Altogether, out findings suggest that the Multi-Spectral
...
meadows \citep{Zoffoli2021, xu2021, Traganos2018, coffer2023}.
...
in therms of reflectance, especially in the visible range
...
were mostly limited to a limited number of flights over a single study
...
Chlorophyceae (Seagrass and Green Algae, respectively). Nine drone
...
accuracy. The classification maps obtained at a very high spatial
resolution with the drone were spatially degraded to satellite
resolutions, making it possible to asses the effect of spatial
resolution on classification accuracy; and provide insights for coastal
habitat mapping using satellite remote sensing.
...
orthomosaicking was identical for every flight. First, tying key points
...
we resampled the drone orthomosaics from their native resolution (8 cm
for the high altitude flights) using the ``average'' method from the
Terra package in R. DISCOV was then applied to these resampled rasters,
and the results were compared to the original model predictions. For
each resolution and vegetation class, we calculated the predicted area
loss, where a score of 0 indicates no area loss during spatial
resampling, and a score of 100 indicates complete loss of the vegetation
class.
...
variable (the loss of vegetation area within a given class). The model
...
with 1.2\% area lost compared to the native resolution (80 mm). As the
...
class. To predict Chlorophyceae with a model likelihood of 0.85, a cover
of 93 \% was needed, 90 \% for Magnoliopsida, 92 \% for Rhodophyceae and
97 \% for Bacillariophyceae. When the vegetation cover of a given class
...
migration to the surface during low tide, consequently altering the
model's classification from bare sediment to Bacillariophyceae.
...
monitoring or homogeneous vegetation types, 10 m resolution of S2/MSI or
...
striking difference for the variable of importance to discriminate
...
potential to perform this discrimination between green these
